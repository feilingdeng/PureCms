...
namespace PureCms.Web.Framework
{
    ...
    public abstract class ThemeVirtualPathProviderViewEngine : VirtualPathProviderViewEngine
    {
        ...
        /// 前台视图位置格式
        /// </summary>
        private readonly string[] _webviewlocationformats = new string[4] { "~/themes/{2}/views/{1}/{0}.cshtml", "~/themes/{2}/views/shared/{0}.cshtml", "~/views/{1}/{0}.cshtml", "~/views/shared/{0}.cshtml" };
        /// <summary>
        /// 插件视图位置格式
        /// </summary>
        private readonly string[] _pluginviewlocationformats = new string[2] { "~/Plugins/{2}/Views/{1}/{0}.cshtml", "~/Plugins/{2}/Views/{1}/shared/{0}.cshtml" };
        ...
        /// 后台视图位置格式
        /// </summary>
        private readonly string[] _adminviewlocationformats = new string[2] { "~/admin/views/{1}/{0}.cshtml", "~/admin/views/shared/{0}.cshtml" };
        ...
        private string GetPath(ControllerContext controllerContext, string name, string controllerName, string cacheKeyPrefix, bool useCache, out string[] searchedLocations)
        {
            ...
            else if (area == "plugin")//插件视图位置的处理
            {
                locations = _pluginviewlocationformats;
            }
        ...
        private string GetPathFromGeneralName(ControllerContext controllerContext, string[] viewLocationFormats, string name, string controllerName, string theme, string cacheKey, ref string[] searchedLocations)
        {
            ...
            {
                //插件的处理
                if (controllerContext.RouteData.Values.ContainsKey("pluginsystemname"))
                {
                    theme = GetRouteDataValue(controllerContext.RouteData, "pluginfoldername", theme);
                    controllerName = GetRouteDataValue(controllerContext.RouteData, "plugincontroller", controllerName);
                    name = GetRouteDataValue(controllerContext.RouteData, "pluginaction", "index") ?? name;
                }
        ...
        private string GetRouteDataValue(RouteData routeData, string key, string defaultValue = "")
        {
