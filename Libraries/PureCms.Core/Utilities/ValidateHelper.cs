...
namespace PureCms.Core.Utilities
{
    /// <summary>
    /// 验证帮助类
    /// </summary>
    public class ValidateHelper
    {
        ...
        public static bool InIP(string sourceIP, string targetIP)
        {
            ...
            string[] sourceIPBlockList = sourceIP.SplitSafe(@".");
            string[] targetIPBlockList = targetIP.SplitSafe(@".");
        ...
        {
            string[] targetIPList = targetIPStr.SplitSafe("\n");
        ...
        public static bool BetweenPeriod(string[] periodList, out string liePeriod)
        {
            if (periodList != null && periodList.Length > 0)
            {
                ...
                foreach (string period in periodList)
                {
                    ...
                    startTime = period.Substring(0, index).ToDateTime(null).GetValueOrDefault();
                    endTime =  period.Substring(index + 1).ToDateTime(null).GetValueOrDefault();
        ...
        {
            string[] periodList = periodStr.SplitSafe("\n");
        ...
        {
            return IsNumericArray(numericRuleStr.SplitSafe(splitChar));
        }
