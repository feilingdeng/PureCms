...
namespace PureCms.Data.User
{
    public class UserRepository : IUserRepository
    {
        ...
        private Sql ParseWhereSql(UserQueryContext q, Sql otherCondition = null)
        {
            ...
            if (q.MobileNumber.IsNotEmpty())
            {
                filter.Append(string.Format("{0} {1}.MobileNumber=@0", optName, TableName), q.MobileNumber);
                optName = " AND ";
            }
            if (q.UserIdList.IsNotNullOrEmpty())
            {
                filter.Append(string.Format("{0} {1}.UserId IN(@0)", optName, TableName), q.UserIdList);
                optName = " AND ";
            }
            ...
            {
                filter.Append(string.Format("{0} {1}.UserId=@0", optName, TableName), q.UserId.Value);
                optName = " AND ";
            }
